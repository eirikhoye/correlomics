...
for directory in os.listdir('mirgenedb_merged_extended_sam/'):
	if not os.path.isdir('mirgenedb_merged_extended_sam/' + str(directory)):
		continue
	if not os.path.exists('mirgenedb_merged_extended_sam_filtered_no_outside_pos2_18_mismatch/'):
		os.makedirs('mirgenedb_merged_extended_sam_filtered_no_outside_pos2_18_mismatch/')
...
#			readID = {k:v for k,v in readID.items() if v != 'remove'}
			...
			if not os.path.exists('mirgenedb_merged_extended_sam_filtered_no_outside_pos2_18_mismatch/' + str(directory)):
				os.makedirs('mirgenedb_merged_extended_sam_filtered_no_outside_pos2_18_mismatch/' + str(directory))
			with open('mirgenedb_merged_extended_sam_filtered_no_outside_pos2_18_mismatch/' + str(directory) + '/' + str(filename) + '.filtered.sam', 'w') as outfile:
				if not os.path.exists('mirgenedb_merged_extended_sam_filtered_no_outside_pos2_18_mismatch/' + str(directory)):
					os.makedirs('mirgenedb_merged_extended_sam_filtered_no_outside_pos2_18_mismatch/' + str(directory))

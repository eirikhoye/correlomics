...
def CountWhobbles(input_fold):
 s   output_list = [0] * len(input_fold)
...
# load paths from SnakeFile
merged_aligned_positions_extended_mature_path = snakemake.input['merged_aligned_positions_extended_mature']
species_dict_path = snakemake.input['species_dict']
stemfile_path = snakemake.input['stemfile']
loopfile_path = snakemake.input['loopfile']
matures_path = snakemake.input['matures']
...
df = pd.read_csv(merged_aligned_positions_extended_mature)
...
with open(species_dict_path) as f:
    data = f.read()

...
#       constraints.write('P' + ' ' + str(loopstart + 1) + ' ' + str(0) + ' ' + str(loopend - loopstart - 2) + '\n') # define loop ds
   ...
   #     pos_5p = [i + self.end_5p_arm for i in [-2, -1, 0, 1]]
   #     pos_3p = [i + self.start_3p_arm for i in [1, 0, -1, -2]]
        ...
        #print(self.hairpin['seq'])
        #print(self.hairpin['dot'])
        #print(dicer_cleave)
        #print(dicer_cleave[3][-2])
        #print(dicer_cleave[3][-3])
        #print(dicer_cleave[2][-4])
...
for file in os.listdir(hairpin_dict):
    ...
    fold.fetch_hairpin()
    fold.find_motif()
    GYM_dict[mirgenedb_id] = fold.GYM


df = pd.DataFrame.from_dict(GYM_dict, orient='index')
df.columns = ['G', 'Y', 'm']
df.to_csv('GYM_motifs.csv')